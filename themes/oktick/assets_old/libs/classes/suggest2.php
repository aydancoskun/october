...
class Suggest {
	...
	function __construct() {
		...
		define("NO_RESULTS_AT_ALL","No results for any Method we tried");
	...
	function ajax($f3, $params){
		...
		if (DEBUG) $f3->get('FP')->log( __CLASS__."->".__FUNCTION__."() \$this->url_query = $this->url_query");
		if ($this->url_query){
			...
			if( ! method_exists( $this, $function ) ){
				...
				if (DEBUG) $f3->get('FP')->log( __CLASS__."->".__FUNCTION__."() Method \$this->$function() does not exist");
		...
		if (! Misc::$suggestions ){
			...
			$this->setResponse(__LINE__);
			if (DEBUG) $f3->get('FP')->log( __CLASS__."->".__FUNCTION__."() \$this->$function() no results");
			return Misc::quit_suggest(__LINE__);
		}
...
//		Misc::set_onSearchStart($onSearchStart, "Start".$line);
		...
		elseif(Misc::$no_suggestions == NO_RESULTS_AT_ALL){
	...
	function bp_esign(){
		Misc::debug_function_start(__CLASS__,__FUNCTION__);
		$f3 = \Base::instance();
		$es = $f3->get('ESIGN');
...
//		$f3->get('FP')->log( __CLASS__."->".__FUNCTION__."() \$sql = $sql");
