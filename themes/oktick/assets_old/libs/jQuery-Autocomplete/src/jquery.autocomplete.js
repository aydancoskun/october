...
//                    dataType: options.dataType
        ...
        get_serviceUrl: function (q) {
            ...
            return str_page_url + '/' + str_page_type_search + '/' + str_page_type_suggest + '/' + q;
        },
...
//            that.el.val(that.getValue(that.suggestions[index].value));
        ...
        onSelect: function (index) {
            ...
            if ($.isFunction(onSelectCallback)) {
            	...
            	if (old_val.length > 1){
					var last_chr = old_val.substring(old_val.length - 1,old_val.length);
					if(last_chr == "+"){
            	...
            	//alert(Selected_val);//"that.currentValue="+that.currentValue);
	                ...
	                that.findBestHint();
	                that.onValueChange();
	            	if (onSelectReturn_val.length > 1){
						last_chr = onSelectReturn_val.substring(onSelectReturn_val.length - 1,onSelectReturn_val.length);
						if(last_chr == "+"){
							onSelectReturn_val = onSelectReturn_val.substring(0,onSelectReturn_val.length - 1);
						}
					}
	            	that.el.val(onSelectReturn_val);
                  	return;
