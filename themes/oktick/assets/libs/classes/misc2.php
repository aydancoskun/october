...
class Misc {
	...
	static function set_ajax_suggestions( $suggestions , $url_query = '', $field_to_extract = 'result' ){
		...
		if(DEBUG) \Base::instance()->get('FP')->log("no_suggestions =".self::$no_suggestions);
	}

	static function render_ajax_suggestions( ){
		if(DEBUG) \Base::instance()->get('FP')->log("num_results_returned=".self::$no_suggestions);
	...
	static function has_trailing_space($string){
			if ( substr( $string , -1 ) == " " ) return self::$url_trailing_space = true;
			return self::$url_trailing_space = false;
	}

	static function has_expansion_sign($string){
		$es = \Base::instance()->get('ESIGN');
		if ( strpos( $string , $es ) !== false) return self::$has_expansion_sign = true;
		return self::$has_expansion_sign = false;
	}
	...
	static function sort_array_with_sort_match_on_top($array,$sort_match){
		...
		if(DEBUG) \Base::instance()->get('FP')->log("\$sort_match=$sort_match");
...
//		if($left_bracket_pos and $rite_bracket_pos and $left_bracket_pos < $rite_bracket_pos) return trim(substr($string, 0, $left_bracket_pos));
	...
	static function quit_suggest($line,$errmsg=""){
		if(DEBUG){
			\Base::instance()->get('FP')->log( __CLASS__."->".__FUNCTION__."('$errmsg'):$line");
		}
		self::$suggestions[]=self::suggest_get_taconite();
		echo self::suggest_json_encode(self::$suggestions);
	...
	static function debug_function_start($class,$function,$text=""){
		if ( ! DEBUG) return;
		$f3 = \Base::instance();
		if($text) $text = ", $text";
		$f3->get('FP')->log( $class."->".$function."() initialized$text");
	}
	...
	static function middle_words($str){
		...
		$c = 0;
		$total = count($parts);
//		if (DEBUG) \Base::instance()->get('FP')->log( "total words = $total" );
	...
	static function get_url_query($params,$start = 3){
		$f3 = \Base::instance();
		$es = $f3->get('ESIGN');
		$url_query = trim($params[0],'/');
		$url_query = Misc::strip_incorrect_chars(Misc::oktick_clean_url($url_query));

		$right_space_pos = strrpos($url_query," ");
		$left_space_pos = strrpos($url_query," ");
		if( $right_space_pos !== false AND $right_space_pos <> $left_space_pos )
			$url_query[$right_space_pos]="+";

		if( $url_query ) {
			Misc::has_trailing_space($url_query);
			$url_query = trim($url_query);
			if (DEBUG) $f3->get('FP')->log('$url_query = "'.$url_query.'"');
			if(Misc::has_expansion_sign($url_query)){
				if (DEBUG) $f3->get('FP')->log('"'.$es.'" detected');
				Misc::$esign_L = trim(substr($url_query, 0, strpos($url_query, $es)));
				Misc::$esign_R = trim(substr(str_replace("+", " ", $url_query), strpos($url_query, $es)+1));
				if (DEBUG) $f3->get('FP')->log( __CLASS__."->".__FUNCTION__."() Misc::\$esign_L = ".Misc::$esign_L);
				if (DEBUG) $f3->get('FP')->log( __CLASS__."->".__FUNCTION__."() Misc::\$esign_R = ".Misc::$esign_R);
				$url_query = trim(str_replace($es, " ", $url_query));
			}
		}
//		if (strpos($url_query, " ") !== false) $this->url_has_space = true;
		if (DEBUG) $f3->get('FP')->log( __CLASS__."->".__FUNCTION__."() \$url_query = $url_query");
		return $url_query;
	}
	...
	static function get_result_query($params){
		$f3 = \Base::instance();
		$url_query = trim($params[0],'/ ');
		$url_query = trim(Misc::strip_incorrect_chars(Misc::oktick_clean_url($url_query)));
