...
class Plugin extends PluginBase
{
    ...
    public function pluginDetails()
    {
        return [
            ...
            //Provides language switching with MaxMind GeoIP Database
            'author' => 'Keios',
            'icon' => 'icon-globe'
        ];
    }
    ...
    {
        if (!App::runningInConsole() && !Request::ajax() && in_array(strtolower(Request::method()), ['get', 'head'])) {
            ...
            if (is_null($locale)) {
                ...
                if (is_array($locale)) {
                    if (count($locale) < 1) {
                        ...
                        Session::push('geolocaleswitcher.localeArray', $locale);
                        $locale = strtolower(array_shift($locale));
                    }
                } else {
                    if (is_string($locale)) {
            ...
            $robotsUri = 'robots.txt';

            if (array_key_exists($locale, $availableLocales) &&
                (strpos(Request::path(), $backendUri) === false) &&
                (strpos(Request::path(), $sitemapUri) === false) &&
                (strpos(Request::path(), $robotsUri) === false) &&
                (!array_key_exists($firstSegment, $availableLocales))
            ) {
