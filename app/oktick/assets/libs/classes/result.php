  ...
  protected $str_suppliers_head       = "";
	...
	function ajax($f3, $params){
		...
		if ( $this->get_prime_data() ){
			$submit=array(
				'str_search_field'					=> Misc::debrace($this->url_query),
	...
	function get_prime_data(){
		...
		$sql = array(
					...
					"c.company_website_proto as proto, ".
					"c.company_website_www as www, ".
					"c.company_website_url AS url, ".
					"bs.bp as BP, ".
					"credit, ".
					"c.uid as cuid, ".
					"count(*) as ccounter, ".
		...
		if (DEBUG) $f3->get('FP')->log($f3->get('DB')->log());
		$this->str_suppliers = "";
		$this->str_suppliers_head = Misc::debrace($prime).$symbol;
